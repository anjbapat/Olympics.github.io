...
host_city <- read_excel("host_city.xls")
cities <- world.cities %>%
  mutate(City = name) %>%
  mutate(region = country.etc)
cities <- cities %>% select(City, region, lat, long)

regioncitylatlong <- left_join(host_city, cities, by=c("City","region"))

#regioncity = unique(df_merge %>% select(Year, City))
regioncitylatlong <- regioncitylatlong %>% 
  group_by(City, region)%>%
  mutate(year = paste0(Year, collapse = ","))
regioncity <- unique(regioncitylatlong %>% select(Year, City))
...
ui <- dashboardPage(skin = "yellow",
  dashboardHeader(title = "Olympic Games Data for 120 years",tags$li(class = "dropdown",tags$a(href = "https://github.com/anjbapat/Olympics.github.io", icon(name= "GitHub",class="fab fa-github"))),
                                                  tags$li(class = "dropdown",tags$a(href = "https://www.olympic.org/", icon(name= "About Olympics",class="fab fa-info-circle"))),
                                                  tags$li(class = "dropdown",tags$a(href = "https://docs.google.com/forms/d/e/1FAIpQLSfTCvwJWXp9CgdG6mQ7kgn06chfwXzZqHUJaobJdWCZMR1keQ/viewform", icon(name= "Feedback",class="fas fa-comments")))),
  ...
  dashboardBody( 
    ...
    tabItems(
      tabItem(tabName = "worldmap", mainPanel(
        h5("Note: Hover the mouse over the country to see the distribution of Gold, Silver and Bronze medals")),
              fluidRow(box(
      ...
      server <- function(input, output) {
        ...
        output$hostcitymap = renderPlotly(
          ggplot(data = HostCityDF, aes(x = long, y = lat)) +
            ...
            geom_point(aes(text=paste(City, ",", region, ":", year)), color = 'red') +
            theme(
              panel.background = element_rect(
                fill = "lightblue",
                colour = "lightblue",
